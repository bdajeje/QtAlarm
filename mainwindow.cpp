  ...
  : QMainWindow(parent)
{
  ...
  // Snooze options
  m_snooze_options = new SnoozeOptions(this, Qt::Dialog);
  m_snooze_options->setWindowModality(Qt::ApplicationModal);
...
void MainWindow::createMenu()
{
  ...
  auto menu_options = menuBar()->addMenu(tr("&Options"));
	...
	// Snooze
	auto action_snooze = new QAction(tr("&Snooze"), this);
	action_snooze->setStatusTip(tr("Update snooze options"));
	action_snooze->setIcon(utils::IconsManager::get("snooze.png"));
	connect(action_snooze, SIGNAL(triggered()), this, SLOT(showSnoozeOptions()));
	menu_options->addAction(action_snooze);
...
void MainWindow::showSnoozeOptions()
{
  m_snooze_options->move( pos().x(), pos().y() );
  m_snooze_options->show();
}
